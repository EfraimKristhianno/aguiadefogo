...
import { downloadMockPDF, generateFolhaPagamentoPDF, generateHoleritePDF } from "@/lib/download-utils";
...
export default function FolhaPagamento() {
  const handleExport = () => {
    downloadMockPDF("Folha_Pagamento_Mar_2026.txt", generateFolhaPagamentoPDF());
    toast.success("Folha de pagamento exportada!");
  };

  const handleHolerite = (name: string) => {
    downloadMockPDF(`Holerite_${name.replace(/\s/g, "_")}.txt`, generateHoleritePDF(name, "Mar/2026"));
    toast.success(`Holerite de ${name} gerado!`);
  };

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold">Folha de Pagamento</h1><p className="text-muted-foreground">Março 2026</p></div>
        <div className="flex gap-2">
          <Button variant="outline" onClick={handleExport}><Download className="h-4 w-4 mr-2" />Exportar</Button>
          <Button className="bg-primary hover:bg-primary/90" onClick={() => toast.success("Holerites gerados com sucesso!")}><Receipt className="h-4 w-4 mr-2" />Gerar Holerites</Button>
        </div>
      </div>
          ...
          <tbody>
            {mockFolha.map((f) => (
              <tr key={f.id} className="border-b border-border/50 hover:bg-muted/20 transition-colors">
                ...
                <td className="py-3 px-4"><Badge className={f.status === "Pago" ? "bg-green-100 text-green-700 hover:bg-green-100" : "bg-yellow-100 text-yellow-700 hover:bg-yellow-100"}>{f.status}</Badge></td>
                <td className="py-3 px-4 text-right"><Button variant="ghost" size="sm" onClick={() => handleHolerite(f.employee)}><FileText className="h-4 w-4 mr-1" />PDF</Button></td>
              </tr>
            ))}
          </tbody>
        </table>
